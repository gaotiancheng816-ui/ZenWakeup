...
const CrescentIcon = ({ size = 120 }: { size?: number }) => (
...
export default function EveningReturnScreen({ onDone }: { onDone?: () => void }) {
  ...
  if (phase === 'enter') {
    return (
      <View style={s.root}>
        ...
        <Animated.View style={[s.btnWrap, { opacity: fadeIn }]}>
  ...
  return (
    <View style={s.root}>
      ...
      <Animated.View style={[s.btnWrap, { opacity: fadeIn, bottom: height * 0.18 }]}>
...
const s = StyleSheet.create({
  ...
  card:         { width:width-64, height:height*0.44, backgroundColor:'rgba(234,230,220,0.88)', borderRadius:8, borderWidth:1, borderColor:'rgba(30,32,48,0.08)', padding:32, justifyContent:'space-between', overflow:'hidden' },
